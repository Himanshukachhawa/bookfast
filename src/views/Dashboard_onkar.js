...
class Dashboard extends Component<Props> {
  ...
  subscription_list = async () => {
    ...
    await axios({
      ...
      .then(async (response) => {
        ...
        this.setState({
          sub_data: response.data.result,
          sub_id: response.data.result[0].id,
          amount: response.data.result[0].amount,
        });
        if (response.data.current_subscription) {
          this.setState({
            current_subscription: response.data.current_subscription,
          });
  ...
  subscribe = async () => {
    ...
    await axios({
      ...
      .catch((error) => {
        alert("Sorry something went wrong dashboard");
        this.setState({ isLoading: false });
      });
  };
  ...
  pick_now = async (zone_id) => {
    ...
    if (
      ...
      if (this.state.active_trip_type == 1) {
        ...
        if (this.props.current_sub_id != 0) {
          this.props.navigation.navigate("ConfirmBooking", {
            vehicle_type: this.state.active_vehicle,
            filter: this.state.filter,
            trip_type: this.state.active_trip_type,
    ...
    return (
      <View>
        ...
        >
          <DialogContent>
            <TouchableOpacity
              onPress={this.close_dialog_box.bind(this)}
              activeOpacity={1}
              style={{
                padding: 10,
                alignItems: "center",
              ...
              <ScrollView style={{ padding: 20 }}>
                ...
                >
                  ...
                  >
                    ...
                    >
                      No worries with surge price, traffic, or time of day, when
                      you are under subscription.
                    </Text>
                  </View>
                </View>
                <View
                ...
                >
                  ...
                  >
                    ...
                    >
                      Book your ride when ever you want and be tension free
                      without payment.
                    </Text>
                  </View>
                </View>
                ...
                >
                  ...
                  >
                    ...
                    >
                      Get your first preference daily services at any time
                ...
                <View style={{ flexDirection: "row" }}>
                  <ScrollView
                    ...
                    showsHorizontalScrollIndicator={false}
                  >
                    {this.state.sub_data.map((row, rowIndex) => {
                      return (
                        <TouchableOpacity
                          onPress={this.change_subscription.bind(
                            this,
                            row.id,
                            row.amount
                          )}
                          style={[
                            this.state.sub_id == row.id
                              ? styles.active_sub_bg
                              : styles.inactive_sub_bg,
                          ]}
                        >
                          <Image
                            source={{ uri: img_url + row.sub_image }}
                            style={{ height: 30, width: 30 }}
                          />
                          <View style={{ margin: 2 }} />
                          <Text
                            style={[
                              this.state.sub_id == row.id
                                ? styles.active_sub_title
                                : styles.inactive_sub_title,
                            ]}
                          >
                            {row.sub_name}
                          </Text>
                          ...
                          <Text
                            style={[
                              this.state.sub_id == row.id
                                ? styles.active_sub_desc
                                : styles.inactive_sub_desc,
                            ]}
                          >
                            {row.validity_label}
                          </Text>
                ...
                >
                  Try out the our subscription plan
                </Text>
                <View style={{ margin: 10 }} />
                <Button
                  title="Subscribe Now"
